...
# ── 2. Configuration ──────────────────────────────────────────────────────────
INPUT_DIR  = "/kaggle/input/fedxgnn/"          # ← change to your dataset name
...
CFG = dict(
    ...
    epochs          = 150,
    lr              = 5e-4,
    weight_decay    = 5e-4,   # was 1e-4 — stronger L2
    alpha           = 0.4,    # 40% reg loss, 60% clf loss (was 0.6)
    train_ratio     = 0.75,
    dropout         = 0.30,   # was 0.15 — stronger dropout
    patience        = 30,     # was 25
)
...
class FocalLoss(nn.Module):
    def __init__(self, alpha_fl=0.75, gamma=2.0):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        # alpha weight: alpha_fl for positives, (1-alpha_fl) for negatives
        at   = torch.where(targets == 1,
                           torch.full_like(prob, self.alpha_fl),
                           torch.full_like(prob, 1 - self.alpha_fl))
        return (at * (1 - pt) ** self.gamma * bce).mean()

_n_pos = float(Y_clf.sum())
_n_neg = float((Y_clf == 0).sum())
print(f"Outbreak class balance  →  pos={int(_n_pos)}  neg={int(_n_neg)}  "
      f"ratio={_n_neg/(_n_pos+1e-8):.1f}:1")
focal_loss = FocalLoss(alpha_fl=0.75, gamma=2.0)
...
def run_epoch(window_list, train=True):
    ...
    with ctx:
        for x_dyn, y_r, y_c in window_list:
            ...
            loss_c = focal_loss(outbreak_prob[mask], y_c[mask])
