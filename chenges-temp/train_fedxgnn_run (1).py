...
# ── 2. Configuration ──────────────────────────────────────────────────────────
# NOTE: Kaggle mounts datasets under /kaggle/input/<dataset-name>/
...
CFG = dict(
    ...
    data_path   = os.path.join(INPUT_DIR, "training_dataset_with_ner.csv"),
    edges_path  = os.path.join(INPUT_DIR, "graph", "graph_edges.csv")
                  if os.path.exists(os.path.join(INPUT_DIR, "graph", "graph_edges.csv"))
                  else os.path.join(INPUT_DIR, "graph_edges.csv"),
    ...
    dropout         = 0.50,   
    patience        = 40,     
    target_log      = True,   # New flag for log(1+y) scaling
)
...
# Normalise regression target (LOG-SCALING)
if CFG.get("target_log"):
    print("Applying log(1+y) transformation to target...")
    df["cases_norm"] = np.log1p(df["cases"])
    # We still center/scale the log-values to keep them near zero for GNN stability
    log_mean = df.loc[train_mask, "cases_norm"].mean()
    log_std  = df.loc[train_mask, "cases_norm"].std() + 1e-8
    df["cases_norm"] = (df["cases_norm"] - log_mean) / log_std
else:
    ...
    log_mean, log_std = cases_mean, cases_std # for checkpointing
...
clf_loss_fn = FocalLoss(alpha=0.75, gamma=2.0)
...
for epoch in range(1, CFG["epochs"] + 1):
    ...
    if vl_auprc > best_val_auprc:
        ...
        torch.save({
            ...
            "cfg":         CFG,
            "cases_mean":  log_mean,   # Stores log_mean if target_log is True
...
best_vl_rmse = min(history["val_rmse"]) if history["val_rmse"] else 0.0
total_epochs_ran = len(history["val_auc"])

metrics_out = {
    "generated_at":       datetime.datetime.now().isoformat(),
    "total_epochs_ran":   total_epochs_ran,
    "best_epoch":         int(history["val_auprc"].index(best_vl_auprc) + 1),
    "early_stop_metric":  "val_auprc",
    "val": {
        "auc_roc":  round(best_vl_auc,   4),
        "auprc":    round(best_vl_auprc,  4),
        "f1":       round(best_vl_f1,     4),
        "mae":      round(best_vl_mae,    4),
        "rmse":     round(best_vl_rmse,   4),
    },
    "train": {
        "best_auc_roc": round(max(history["train_auc"])   if history["train_auc"]   else 0.0, 4),
        "best_auprc":   round(max(history["train_auprc"]) if history["train_auprc"] else 0.0, 4),
        "best_f1":      round(max(history["train_f1"])    if history["train_f1"]    else 0.0, 4),
    },
    "config": {
        ...
        "loss":          "FocalLoss(alpha=0.75, gamma=2.0) + MSELoss",
        "optimizer":     "AdamW",
        "n_districts":   N_NODES,
        "n_time_steps":  N_TIME,
    },
    "history": {
        ...
        "val_auprc": [round(v, 4) for v in history["val_auprc"]],
        "val_f1":    [round(v, 4) for v in history["val_f1"]],
        "val_mae":   [round(v, 4) for v in history["val_mae"]],
...
print(f"  Val MAE     : {best_vl_mae:.4f}")
print(f"  Best epoch  : {metrics_out['best_epoch']} / {total_epochs_ran}")
